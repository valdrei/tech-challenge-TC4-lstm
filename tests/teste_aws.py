...
# URL da API AWS (pegue do output do Terraform ou console AWS)
# Exemplo: http://lstm-alb-123456789.sa-east-1.elb.amazonaws.com
if len(sys.argv) > 1:
    API_URL = sys.argv[1].rstrip('/') + '/predict'
else:
    API_URL = input("Digite a URL do Load Balancer AWS (ex: http://lstm-alb-xxx.sa-east-1.elb.amazonaws.com): ").rstrip('/') + '/predict'

print("=" * 70)
print(f"🌐 TESTANDO API NA AWS COM {SYMBOL}")
print("=" * 70)
print(f"URL: {API_URL}")
...
try:
    # Adicionar timeout maior para AWS
    ...
    print("\n❌ Erro: Não foi possível conectar ao Load Balancer!")
    print(f"   URL testada: {API_URL}")
    print("\n   Verifique:")
    print("   1. Load Balancer está rodando na AWS?")
    print("   2. Security Group permite tráfego na porta 8000?")
    print("   3. Target Group está healthy?")
    print("   4. URL está correta?")
except requests.exceptions.Timeout:
    print("\n❌ Erro: Timeout ao conectar (> 30s)")
    print("   A API pode estar demorando muito para responder")
